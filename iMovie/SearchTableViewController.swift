...
class SearchTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    var arrayFilms = [FilmList]()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        let search = UISearchController()
        search.searchBar.barTintColor = .green
    ...
    func searchFilmByName (title: String){
        SVProgressHUD.show()
        let parameters = ["api_key": apiKey, "language": "ru-RU", "query": title] as [String: Any]
                          
        AF.request("https://api.themoviedb.org/3/search/movie", method: .get, parameters: parameters).responseData {response in
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        arrayFilms.removeAll()
        tableView.reloadData()
        searchFilmByName(title: searchBar.text!)
    }
